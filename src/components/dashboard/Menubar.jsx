...
export default function Menubar({ authenticatedUser }) {
    ...
    return (
        <div className='col-span-2 select-none lg:col-span-1 h-screen top-0 left-0 px-4 absolute bg-stone-700 lg:relative border-r border-stone-500 shadow-lg'>
            <ul className=''>
                ...
                { isDashMenuOpen &&
                    <div className='my-12 lg:my-2'>
                        ...
                        <li className='px-4 py-4 cursor-pointer text-slate-100  text-xl lg:text-2xl text-center bg-stone-800 duration-400 ease-in-ease-out border border-stone-600 shadow-lg rounded-md'>{ authenticatedUser?.name }</li>
                        { profileMenus.map(menu => (
                            <li key={ menu.id } onClick={ () => makeMenuActivated(menu.id) } className={ `px-4 my-1 py-2 text-slate-100 text-xl hover:bg-stone-800 ${activatedMenu == menu.id ? 'bg-stone-800' : ''} duration-400 ease-in-ease-out cursor-pointer text-center border border-stone-600 shadow-lg rounded-md` }>{ menu.title }</li>
                        ))
                        }
                    </div>
                }
